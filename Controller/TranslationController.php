...
class TranslationController extends Controller
{
    ...
    public function exportChangesAction(Request $request)
    {
        ...
        $process->run(
            function ($type, $buffer) use ($output) {
                $txt = (Process::ERR === $type) ? 'ERR:' . $buffer : $buffer;
                fwrite($output, $txt);
            }
        );

        $message = $this->get('translator')->trans('translations.export_changes_success', array(), 'LexikTranslationBundle');
    ...
    public function importTranslationsAction(Request $request)
    {
        ...
        $message = $this->get('translator')->trans('translations.import_translations_success', array(), 'LexikTranslationBundle');
